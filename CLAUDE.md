...
Alongside the strict correction, every submit fires a **second, parallel** provider call using `SYSTEM_PROMPT`'s counterpart `SUGGESTION_PROMPT` (`src/prompts/suggestionPrompt.ts`). Where the corrector is forbidden from rewriting, the suggester proposes native-speaker phrasing (articles, prepositions, collocations, idioms, word choice) while preserving meaning, structure, tone, and language. It also applies the same plain grammar/spelling/punctuation fixes the corrector makes, so the suggested text reads as correct *and* natural and never contradicts a strict proofread — but since the two calls are independent (and not chained), agreement is soft, not guaranteed. It wraps its reply in `<suggested>…</suggested>` plus an optional `<notes>` block, parsed by the same `parseOutput` (now taking a tag argument: `'corrected'` | `'suggested'`).

Both prompts are user-overridable from the SettingsPanel: `Settings.systemPrompt` and `Settings.suggestionPrompt` (both optional in `grammar.settings.v1`; absent = use the built-in default). Each has its own "Suggestion prompt" / "System prompt" textarea with a "Restore default" button (sets the field back to `undefined`). The override is threaded through `correct`/`retry` and applied in `runCorrection`/`runSuggestion`, which fall back to the built-in constant when the override is empty/blank. A custom suggestion prompt **must** still emit `<suggested>…</suggested>` — otherwise `parseOutput('suggested')` falls back to treating the whole response as the suggestion (same fragility as the corrector).
